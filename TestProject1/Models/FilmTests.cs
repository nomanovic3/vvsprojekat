...
namespace FilmMate.Tests.Models
{
    ...
    public class FilmTests
    {
        ...
        [TestMethod]
        public void DodajOcjenu_ValidRating_AddsToList()
        {
            var film = new Film("Test", "Drama", 5.0, 2020);
            int initialCount = film.getOcjene().Count;

            film.DodajOcjenu(8);

            Assert.AreEqual(initialCount + 1, film.getOcjene().Count);
        }

        [TestMethod]
        public void GetOcjena_NoRatings_ReturnsZero()
        {
            var film = new Film("Test", "Drama", 5.0, 2020);
            film.getOcjene().Clear();

            Assert.AreEqual(0.0, film.getOcjena(), 0.001);
        }

        [TestMethod]
        public void ToString_ValidFilm_ReturnsFormattedString()
        {
            var film = new Film("Matrix", "Sci-Fi", 9.0, 1999);
            string result = film.ToString();

            StringAssert.Contains(result, "Matrix");
            StringAssert.Contains(result, "Sci-Fi");
            StringAssert.Contains(result, "1999");
        }

        [TestMethod]
        public void SetNaziv_EmptyString_SetsValue()
        {
            var film = new Film("Original", "Drama", 5.0, 2020);
            film.setNaziv("");

            Assert.AreEqual("", film.getNazivFilma());
        }

        [TestMethod]
        public void SetKategorija_NullString_SetsValue()
        {
            var film = new Film("Test", "Original", 5.0, 2020);
            film.setKategorija(null);

            Assert.IsNull(film.getKategorija());
        }
